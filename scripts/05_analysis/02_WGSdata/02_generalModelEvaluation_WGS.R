...
tissueCols = c(tissueCols[WGStissues], 
               "generalModel" = "darkgreen")
t2T = c(t2T[WGStissues],"generalModel" = "All-tissue general model")
...
# trimTypes = sapply(WGStissues, function(tissue){
...
# # plot
# png(paste0("fig/MutationOverview_generalModel_WGS", plotEnding, ".png"),
...
#           names.arg="",cex.names=0.5, cex.axis = 1.1,main  = c(t2T[WGStissues],"Combined")[i],
...
# plotDump = sapply(WGStissues, function(tissue){
...
# # AUCs
# AUCs = c(sapply(WGStissues, function(tissue){
...
dumpVar =  sapply(WGStissues, function(tissue){
  ...
  predPerChr = sapply(names(chrCols), function(cr){
    ...
    yHat = predict(rf,data=testDat, num.threads=nThread)
  ...
  perf = performance(temp,"auc")
  auc  =perf@y.values[[1]]
  save(auc, file = paste0("data/Modeling/WholeGenomeData/CrossTissue/generalModel_on_", tissue, ".RData"))
  return(auc)
})
crossTissuePerformance =  sapply(WGStissues, function(tissue){
  load(paste0("data/Modeling/WholeGenomeData/CrossTissue/generalModel_on_", tissue, ".RData"))
  return(auc)
})
...
load("data/Modeling/WholeGenomeData/RF/ROC_PR_RF_concat.RData")
tissueSpecificPerformance = sapply(WGStissues, function(tissue){
  print(tissue)
  ROC_PR_RF_concat[[tissue]]$auc@y.values[[1]]
})
