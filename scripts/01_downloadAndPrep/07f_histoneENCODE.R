...
dumpVar = sapply(targets, function(target){
  ...
  tissueAvail = file.exists(paste0("data/predictors/histoneENCODE/",
  ...
  dumpVar2 = sapply(tempTissues, function(tissue){
    load(paste0("data/predictors/histoneENCODE/",
    ...
    export(gr, con = paste0("temp/histone_", target, "_", tissue, ".bed.gz"))
    return(NULL)
  })
  # merge overlapping intervals
  fixOverlapping = paste0("bedtools merge  -c 5 -o mean -d -1 ",
                          "-i temp/histone_", target, "_", tempTissues, ".bed.gz ",
                          "> temp/histone_", target, "_", tempTissues, "_merged.bedgraph ")
  cmd=paste(fixOverlapping, collapse = "\n")
  system(cmd)
  # combine all GRL elements and read in R
  unionbedg =  paste0("bedtools unionbedg -filler NA -i ", 
                      paste(" temp/histone_", target, "_", tempTissues, "_merged.bedgraph", 
                            sep = "", collapse = " "))
  comb = read.table(text = system(unionbedg, intern = T))  
  file.remove(c(paste0("temp/histone_", target, "_", tempTissues, ".bed.gz"), 
                paste0("temp/histone_", target, "_", tempTissues, "_merged.bedgraph")))
  # convert into a granges object
  val = rowSums(comb[,-(1:3)], na.rm = T) / (ncol(comb)-3)
  gr = GRanges(seqnames=comb$V1,
               ranges=IRanges(start=comb$V2+2, end = comb$V3), 
               val = val)
  save(gr, file = paste0("data/predictors/histoneENCODE/", target, "_", "allTissues.RData"))
})
