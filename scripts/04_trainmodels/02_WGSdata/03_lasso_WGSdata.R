...
tissues = c("brain", "breast", "esophagus", "kidney", "liver",
            "ovary", "prostate", "skin") 
tissue = tissues[args]
dir.create("data/Modeling/WholeGenomeData/Lasso",showWarnings=F)
...
predictions = lapply(chroms, function(cr){
  ...
  sp = stabpath(x = trainX, y = trainY, family = "binomial") #, mc.cores=nThread
  stab = stabsel(sp, error = 0.1, type = "pcer", pi_thr = 0.6)
  
  save(sp, stab, file = paste0("data/Modeling/WholeGenomeData/Lasso/",
                               tissue, "_", cr, ".RData"))
  ...
  save(logR, sigFeatures, file = paste0("data/Modeling/WholeGenomeData/Lasso/", 
...
save(predictions, file = paste0("data/Modeling/WholeGenomeData/Lasso/", 
...
save(feature_selection, file = paste0("data/Modeling/WholeGenomeData/Lasso/", 
...
sp = stabpath(x = X, y = Y, family = "binomial") #, mc.cores=nThread
stab = stabsel(sp, error = 0.1, type = "pcer", pi_thr = 0.6)
save(sp, stab, file = paste0("data/Modeling/WholeGenomeData/Lasso/", 
                             tissue, ".RData"))
