...
library(Biostrings)
dir.create("data/MutTables/exomeTrainData_subsampledNsamples/", showWarnings = F)
#####
...
# get n samples per tissue #####
nSamps = sapply(names(tissue2Cancer), function(tissue){
  ...
  length(table(submuts$Tumor_Sample_Barcode))
})
nSamp = min(nSamps)
save(nSamps,nSamp, file = ("data/MutTables/exomeTrainData_subsampledNsamples/nSamps.RData"))
#####

# for each tissue #####
set.seed(234)
dumpVar = sapply(names(tissue2Cancer), function(tissue){
  print(tissue)
  submuts = muts[muts$cancerType %in% tissue2Cancer[[tissue]],]
  ...
  # subsample
  samps = sample(unique(submuts$Tumor_Sample_Barcode), nSamp)
  submuts = submuts[submuts$Tumor_Sample_Barcode %in% samps,]
  ...
  MutsBed = cbind(Muts[,1], as.integer(Muts[,2]-1), as.integer(Muts[,2]), 
                  ids)
  save(Muts, file = paste0("data/MutTables/exomeTrainData_subsampledNsamples/", tissue, "_Muts.RData"))
  write.table(MutsBed, file = paste0("data/MutTables/exomeTrainData_subsampledNsamples/", tissue, "_Muts.bed"),
