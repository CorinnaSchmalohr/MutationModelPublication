...
pred = mapPredictors(x=tab, 
                     posFile=paste0("data/MutTables/exomeTrainData_subsampledNsamples/", 
                                    tissue, "_Muts.bed"))
load(paste0("data/MutTables/exomeTrainData_subsampledNsamples/", tissue, "_Muts.RData"))
data = list(meta = tab, pred = pred, muts = Muts)
save(data, file = paste0("data/MutTables/exomeTrainData_subsampledNsamples/", tissue, "_Muts_mapped.RData"))

dat = cbind(data$pred, mutated = as.factor(data$muts$mutated))
datchroms = data$muts$chr
dat = as.data.frame(dat)
save(dat,datchroms, file=paste0("data/MutTables/exomeTrainData_subsampledNsamples/", 
